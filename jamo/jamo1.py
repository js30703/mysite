__all__ = ["split_syllable_char", "split_syllables",
           ...
           "CHAR_INITIALS", "CHAR_MEDIALS", "CHAR_FINALS" ,"is_hangul_syllable(c)"]

           
# split_syllables('읽다') 'ㅇㅣㄺㄷㅏ'
# split_syllables('쉬다') 'ㅅㅟㄷㅏ'

# join_jamos('ㅅㅜㅣㄷㅏ') '수ㅣ다'
# join_jamos('ㅇㅣㄹㄱㄷㅏ') '일ㄱ다
# join_jamos('ㄺㅏ') 'ㄺㅏ'
...
def is_Vow(han):
    return  ord('ㅏ') <= ord(han) <= ord('ㅣ')

def get_stem(verb):
    return split_syllables(verb)[:-2]

def end_Vow(verb):
    a = get_stem(verb)
    return ord('ㅏ') <= ord(a[-1]) and ord(a[-1]) <= ord('ㅣ')
    
def A_EOYO(verb):
    
    if get_stem(verb)[-2] == 'ㅇ' and get_stem(verb)[-1] == 'ㅣ':
        b = get_stem(verb)[-3]
        if is_Vow(b) :
            a = get_stem(verb)[:-1] + split_syllables('ㅖ요')        
            return join_jamos(a)
        else: 
            a = get_stem(verb)[:-1] + split_syllables('ㅣ에요')        
            return join_jamos(a)
    
    elif get_stem(verb)[-2] == 'ㅎ' and get_stem(verb)[-1] == 'ㅏ':
        a = get_stem(verb)[:-1] + split_syllables('ㅐ요')        
        return join_jamos(a)
    # 어간의 규칙 활용
    # 어간의 마지막 모음이 ㅏ ㅗ 이면 아요
    # 어간의 마지막 모음이 ㅏ,ㅗ가 아니면 이면 어요
    
    elif end_Vow(verb) : # 모음으로 끝나면

        if get_stem(verb)[-1] =='ㅗ' : # 뒤에서 첫번째 글자로 판단
            a = get_stem(verb)[:-1] + split_syllables('ㅘ요')        
            return join_jamos(a)

        elif get_stem(verb)[-1] =='ㅜ' : # 뒤에서 첫번째 글자로 판단
            a = get_stem(verb)[:-1] + split_syllables('ㅝ요')        
            return join_jamos(a)

        elif get_stem(verb)[-1] =='ㅡ' : # 뒤에서 첫번째 글자로 판단
            a = get_stem(verb)[:-1] + split_syllables('ㅓ요')        
            return join_jamos(a)

        elif get_stem(verb)[-1] =='ㅣ' : # 뒤에서 첫번째 글자로 판단
            a = get_stem(verb)[:-1] + split_syllables('ㅕ요')        
            return join_jamos(a)

        else : 
            a = get_stem(verb) + split_syllables('요')        
            return join_jamos(a)
    else:
        if get_stem(verb)[-2] == 'ㅏ' or get_stem(verb)[-2] =='ㅗ' : # 뒤에서 2번째 글자로 판단
            a = get_stem(verb) + split_syllables('아요')        
            return join_jamos(a)
        else:
            a = get_stem(verb) + split_syllables('어요')        
            return join_jamos(a)